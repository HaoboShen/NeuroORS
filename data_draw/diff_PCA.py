...
# 定义CSV文件路径
file_paths = ['E-nose data/orange_B_14400.csv', 'E-nose data/peach_B_14400.csv']
# file_paths = ['apple_2500.csv', 'orange_2500.csv', 'peach_2500.csv', 'air_2500.csv']
# 数据集标签
labels = ['orangeB', 'PeachB']
...
for file_path in file_paths:
    ...
    # 对数据进行差分处理
    sensor_data_diff = np.diff(sensor_data, axis=0)
    
    row_len.append(sensor_data_diff.shape[0])

    print(row_len)
    if all_data.shape[0] == 0:
        all_data = sensor_data_diff[3600:]
    else:
        all_data = np.vstack((all_data,sensor_data_diff[3600:])) 
