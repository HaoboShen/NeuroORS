...
class teach(nn.Module):
    ...
    def forward(self,x):
        ...
        self.spk,self.syn,self.mem=self.teach(x*0.05, self.syn, self.mem)
...
class ReSuMe(nn.Module):
    def __init__(self, N, out_sz, alpha, beta, train_flag=False):
        super().__init__()
        ...
        self.stdp_learn = STDP(self.readout.readout_in_syn, self.teach.teach,2,2,f_pre=self.f_weight, f_post=self.f_weight)
