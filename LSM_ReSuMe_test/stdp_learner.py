...
def stdp_linear_single_step(
    ...
    # delta_w_post = f_post(weight) * (trace_pre.unsqueeze(1) * out_spike[0].unsqueeze(2)).sum(0)
    print("delta_w_pre:",delta_w_pre)
    # print("delta_w_post:",delta_w_post)
    return trace_pre, trace_post, delta_w_pre 
...
class STDPLearner(nn.Module):
    ...
    def forward(self, on_grad: bool = True, scale: float = 1.):
