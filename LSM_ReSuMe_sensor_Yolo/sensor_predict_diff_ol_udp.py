...
def udp_send():
    ...
    while udp_thread_running:
        server_socket.sendto(bytes([count_result]), server_address)
        time.sleep(0.5)
    server_socket.close()
...
# online learning setting
    load_model_name = "lsm_net_diff_yolo_goodbackup.pth"
    teach_flag = False
...
# lsm network
    ...
    lsm_net.load_state_dict(torch.load(load_model_name,map_location=device))
    lsm_net.eval()
    print(lsm_net)
# result list
    ten_results=[]
...
# print("lsm_out:", lsm_out)
    while ser.receive_thread_running:
        if ser.data_queue.qsize() > 0:
            ...
            # print("data_size",ser.data_queue.qsize())
            ...
            print("lsm_time:",end_time-start_time)
