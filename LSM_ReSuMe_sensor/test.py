...
dataset = CSV2Dataset('orange_peach_A_2500/',num_steps=10)
...
targets = targets_to_spike(targets,num_steps=10,out_sz=4).to(device)
alpha = np.float32(np.exp(-1/16))
beta = np.float32(1 - 1/16)

diff_collect = torch.empty(0).to(device)
lsm_net = LSM(1000, data.shape[-1], 3, np.float32(np.zeros((1000,data.shape[-1]))), np.float32(np.zeros((1000,1000))), alpha=alpha, beta=beta, th=20).to(device)
lsm_net.load_state_dict(torch.load("lsm_net_orange_peach_oline.pth"))
